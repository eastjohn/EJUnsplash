...
class UnsplashSearchServiceTests: XCTestCase {
    ...
    func testFetchDatas_WhenSecondFetching_ThenCallSendRequestOfNetworkManager() {
        let expectedReqeust = UnsplashRequest(api: .search, parameters:[.page:"2", .per_page:"10", .query:"test"])
        sut.currentPage = 1
    ...
    func testFetchDatas_WhenPerPageIs30_ThenRequestPerPageIs30() {
        let expectedReqeust = UnsplashRequest(api: .search, parameters:[.page:"1", .per_page:"30", .query:"test"])
        sut.perPage = 30
    ...
    func testFetchDatas_WhenQueryIsEmpty_ThenNotCallSendRequestOfNetworkManager() {
        let notCalled = "called sendRequest(_:completionHandler:)"
        sut.query = ""
    ...
    func testReceiveData_ThenCallUpdateHandler() {
        ...
        let jsonData = JsonDataCreator.createHasOneSearchData()
        whenFetchData(jsonData: jsonData)
    ...
    func testReceiveData_WhenHasTenDatas_ThenCallUpdateHandler() {
        ...
        whenFetchData(jsonFile: "Search1.json")
    ...
    func testFetchData_ThenNotRetainCylcle() {
        ...
        sut.fetchDatas()
        sut = nil
        networkManagerStub.completionHandler?(JsonDataCreator.createHasOneSearchData(), nil)
